...
class GSN_sparse(nn.Module):
    ...
    def __init__(self,
        ...
        if self.msg_kind == 'gin':
            ...
            update_input_dim = d_in + d_msg + d_msg
    ...
    def forward(self, x, edge_index, **kwargs):
        ...
        if self.msg_kind == 'gin':
            ...
            #                      self.propagate(edge_index=edge_index, x=x, identifiers=identifiers, flow_dir = 'source_to_target')) 
            
            out = self.update_fn((1 + self.eps) * self_msg + 
                                 (1 - alpha) * self.propagate(edge_index=edge_index, x=x, identifiers=identifiers, flow_dir = 'source_to_target') +
                                 alpha * self.propagate(edge_index=edge_index, x=x, identifiers=identifiers, flow_dir = 'target_to_source')) 
                
        elif self.msg_kind == 'general':
            out = self.update_fn(torch.cat((x, (1 - alpha) * self.propagate(edge_index=edge_index, x=x, identifiers=identifiers, flow_dir = 'source_to_target'), 
                                            alpha * self.propagate(edge_index=edge_index, x=x, identifiers=identifiers, flow_dir = 'target_to_source')), -1))
            
            # out = self.update_fn(torch.cat((x, self.propagate(edge_index=edge_index, x=x, identifiers=identifiers, flow_dir = 'source_to_target')), -1))
    ...
    def propagate(self, edge_index, x, identifiers, flow_dir):
        
        select = 0 if flow_dir == 'target_to_source' else 1 
        aggr_dim = 1 - select
